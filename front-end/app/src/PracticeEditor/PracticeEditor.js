...
class PracticeEditor extends Component {
    state = {
        language: "JS",
        languageList: ["JS", "CSharp "]
    }
    componentDidMount() {
        fetch('http//:localhost:3030/problem/' + this.props.match.id)
            .then(data => JSON.parse(data))
            .then(data => this.setState({ problem: data }))
    }
    submit() {
        axios.post(this.serverUrl + '/compile', {code:this.state.code,language:this.state.language})
            .then(data => JSON.stringify(data))
            .then(data => this.setState({ output: data }))
    }
    handleChange = (code) => this.state({ code: code });
                ...
                <div className="code-editor-wrapper">
                    <div className="editor-header">
                        ...
                        <select className="editor-lang-select">
                            {this.state.languageList.map((language) => <option value={language.value}>{language.displayName}</option>)}
                        </select>
                    </div>
